-- ============================================================
-- Seeat 백엔드 DB 스키마 (수산물 거래 플랫폼) - V4
-- 기준: DB_schema_v3.docx + DB_Schema_V4_Review.pdf (2026-07-25)
-- 대상: MySQL 8.0
-- V3 대비 변경 3건 (v2.1 API 명세서 확정 정책 반영):
--   1. order.order_status 허용값에 CANCELLED 추가 (주문 취소)
--   2. product.reserved_quantity 컬럼 추가 (재고 홀드/해제)
--   3. member에 reset_token / reset_token_expires_at / email_verified 추가
--      (비밀번호 재설정, 이메일 인증)
-- 그 외 18개 테이블 / 26개 관계 구조는 V3과 동일. 정규화 검토 및
-- 인덱스·제약조건 설계는 문서 기준대로 이번 범위에서 제외.
-- 사용법: 신규로 스키마를 만들 때 이 파일을 사용. 이미 V3로 만들어진
-- DB에는 이 파일 대신 schema_v4_migration.sql(ALTER)을 적용할 것.
-- ============================================================
-- Docker 버전: docker-compose.yml의 MYSQL_DATABASE 환경변수로 DB가 이미 생성/선택된 상태에서 실행되므로 CREATE DATABASE/USE 없음
...
CREATE TABLE `member` (
    ...
    reset_token              VARCHAR(255) NULL COMMENT '🆕 V4: 비밀번호 재설정 토큰',
    reset_token_expires_at   DATETIME     NULL COMMENT '🆕 V4: 재설정 토큰 만료일시',
    email_verified           BOOLEAN      NOT NULL DEFAULT FALSE COMMENT '🆕 V4: 이메일 인증 여부',
...
CREATE TABLE product (
    ...
    reserved_quantity      INT           NOT NULL DEFAULT 0 COMMENT '🆕 V4: 결제 대기 중 예약(홀드)된 수량',
...
CREATE TABLE `order` (
    ...
    order_status      VARCHAR(30)   NOT NULL COMMENT 'PAYMENT_PENDING/PAYMENT_COMPLETED/PREPARING/SHIPPING/DELIVERED/PURCHASE_CONFIRMED/CANCELLED',
